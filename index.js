...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]} Seja bem-vindo ao grupo:\n*${mdata.subject}*`
				...
				teks = `Tchau @${num.split('@')[0]} Fraco não aguentou a pressão e saiu! 😝👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⏰ calma porra, tô fazendo... aguarde um pouco! ⏰',
				success: '✔️ Funcionou! ✔️',
				error: {
					stick: '⚠️ Falha, ocorreu um erro ao converter a imagem em um adesivo! ⚠️',
					Iv: '❌ Link inválido! ❌'
				},
				only: {
					...
					admin: '😎 Este comando só pode ser usado pelo adm! 😝',
					Badmin: '❌ Este comando só pode ser usado quando eu virar adm nessa porra! 🖕🏻❌'
				}
			}
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'listablock':
					teks = 'Esta é a lista de contatos bloqueados :\n'
				...
				case 'sticker':
					...
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Qual é o código de linguagem que vc quer?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Cadê o texto?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('Coloque o texto um pouco menor!')
				...
				case 'meme':
                 ...
                 case 'dono':
					...
					buffer = await getBuffer(`https://rap24horas.com.br/wp-content/uploads/2020/02/sidoka-2020-fmf.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*NOME:* ঔ͜͡C3N͢NA🌹㝔\n*WPP:* wa.me/+558881418150\n*INSTA:* c3nna_ofc'})
					break
                  case 'lacoste':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSYPZ-M0d8RijPwZmZw7KKspQmaMJBABNj6fw&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Lindão 🐊❤️'})
					break
                  case 'gato':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://static.poder360.com.br/2020/04/GATO-CORONAVIRUS-868x644.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '❤️'})
					break
                  case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://static.mundohentaioficial.com/covers/pack-animes-hentai-vol1-mobile.jpg?rev=1556336061352`)
				...
				case 'clearall':
					...
					reply('Excluir todos os chats com sucesso :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é você?')
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse lek aqui @${mentioned[0].split('@')[0]} Agora é adm do grupo, respeitem ok?! 😎👌🏻`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse lek aqui @${mentioned[0].split('@')[0]} Acabou de perder o adm KKKKK pressionem F aí chat! 😎👌🏻`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('A quem você deseja adicionar?')
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar membro!')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						...
						mentions(`KKKKKKKK acabou de levar ban trouxa, volte sempre : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('⚠️ marque a figurinha! ⚠️')
					reply(mess.wait)
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Cadê o texto man?')
					teks = body.slice(5)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Desativado o modo simi com sucesso neste grupo 📌')
					} else {
